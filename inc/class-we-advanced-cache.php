...
class WE_Advanced_Cache {
	...
	public function set_comment_cookie_exceptions( $comment ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			setcookie( 'we_commented_posts[' . $post_id . ']', parse_url( get_permalink( $post_id ), PHP_URL_PATH ), ( time() + HOUR_IN_SECONDS * 24 * 30 ) );
		}
	}
	...
	public function purge_post_on_comment_status_change( $comment_id ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			$path = untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache/' . preg_replace( '#https?://#i', '', get_permalink( $post_id ) );
	...
	public function print_notice() {
		...
		if ( defined( 'WE_ADVANCED_CACHE' ) && WE_ADVANCED_CACHE ) {
		...
		<div class="error">
			<p>
				<?php if ( $config_file_bad ) : ?>
					<?php esc_html_e( 'define("WP_CACHE", true); is not in wp-config.php.', 'easy-cache' ); ?>
				<?php endif; ?>

				<?php if ( $advanced_cache_file_bad ) : ?>
					<?php esc_html_e( 'wp-content/advanced-cache.php was edited or deleted.', 'easy-cache' ); ?>
				<?php endif; ?>

				<?php esc_html_e( 'easy Cache is not able to utilize page caching.', 'easy-cache' ); ?>
	...
	public function clean_up() {
		...
		$folder = untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache';
	...
	public function write() {
		...
		if ( ! empty( $config['enable_page_caching'] ) ) {
			...
			"\n\r" . "define( 'WE_ADVANCED_CACHE', true );" .
			"\n\r" . 'if ( is_admin() ) { return; }' .
			"\n\r" . "if ( ! @file_exists( WP_CONTENT_DIR . '/sc-config/config-' . \$_SERVER['HTTP_HOST'] . '.php' ) ) { return; }" .
			"\n\r" . "\$GLOBALS['we_config'] = include( WP_CONTENT_DIR . '/sc-config/config-' . \$_SERVER['HTTP_HOST'] . '.php' );" .
			"\n\r" . "if ( empty( \$GLOBALS['we_config'] ) || empty( \$GLOBALS['we_config']['enable_page_caching'] ) ) { return; }" .
	...
	public function toggle_caching( $status ) {
		...
		array_unshift( $config_file, '<?php', "define( 'WP_CACHE', $status_string ); // easy Cache" );
