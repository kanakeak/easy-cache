...
class WE_Config {
	...
	public function write( $config ) {
		...
		$config_dir = WP_CONTENT_DIR . '/we-config';
	...
	public function get() {
		...
		$config = get_option( 'we_easy_cache', $this->get_defaults() );
	...
	public function verify_file_access() {
		...
		// If the cache and/or cache/easy-cache directories exist, make sure it's writeable.
		if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache' ) ) {
			if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/cache' ) ) {
				return false;
			}

			if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache' ) ) {
				if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache' ) ) {
					return false;
				}
			}
		}

		// If the we-config directory exists, make sure it's writeable.
		if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/we-config' ) ) {
			if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/we-config' ) ) {
				return false;
			}
		}
	...
	public function clean_up() {
		...
		$folder = untrailingslashit( WP_CONTENT_DIR ) . '/we-config';

		delete_option( 'we_easy_cache' );
