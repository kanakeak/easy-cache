...
class WE_Cron {
	...
	private function setup() {
		...
		add_action( 'we_purge_cache', array( $this, 'purge_cache' ) );
	...
	public function filter_cron_schedules( $schedules ) {
		...
		$schedules['easy_cache'] = array(
			'interval' => apply_filters( 'we_cache_purge_interval', $interval ),
			'display'  => esc_html__( 'easy Cache Purge Interval', 'easy-cache' ),
		);
		return $schedules;
	}
	...
	public function unschedule_events() {
		$timestamp = wp_next_scheduled( 'we_purge_cache' );

		wp_unschedule_event( $timestamp, 'we_purge_cache' );
	}
	...
	public function schedule_events() {
		...
		if ( ! $timestamp ) {
			wp_schedule_event( time(), 'easy_cache', 'we_purge_cache' );
		}
	}
