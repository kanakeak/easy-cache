 ...
 * @return string
 */
function we_cache( $buffer, $flags ) {
	global $post;
	...
	if ( ! $filesystem->exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache' ) ) {
		if ( ! $filesystem->mkdir( untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache' ) ) {
			// Can not cache!
			return $buffer;
		}
	}

	$buffer = apply_filters( 'we_pre_cache_buffer', $buffer );

	$url_path = we_get_url_path();

	$dirs = explode( '/', $url_path );

	$path = untrailingslashit( WP_CONTENT_DIR ) . '/cache/easy-cache';
	...
	if ( preg_match( '#</html>#i', $buffer ) ) {
		$buffer .= "\n<!-- Cache served by Easy Cache - Last modified: " . gmdate( 'D, d M Y H:i:s', $modified_time ) . " GMT -->\n";
	}

	if ( ! empty( $GLOBALS['we_config']['enable_gzip_compression'] ) && function_exists( 'gzencode' ) ) {
	...
	if ( function_exists( 'ob_gzhandler' ) && ! empty( $GLOBALS['we_config']['enable_gzip_compression'] ) ) {
 ...
 * @since 1.0
 */
function we_serve_cache() {
	$file_name = 'index.html';

	if ( function_exists( 'gzencode' ) && ! empty( $GLOBALS['we_config']['enable_gzip_compression'] ) ) {
		$file_name = 'index.gzip.html';
	}

	$path = rtrim( WP_CONTENT_DIR, '/' ) . '/cache/easy-cache/' . rtrim( we_get_url_path(), '/' ) . '/' . $file_name;
 ...
 * @return boolean
 */
function we_url_exception_match( $exception, $regex = false ) {
