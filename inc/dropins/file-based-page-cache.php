...
if ( ! empty( $_COOKIE ) ) {
	...
	if ( ! empty( $_COOKIE['WE_commented_posts'] ) ) {
		foreach ( $_COOKIE['WE_commented_posts'] as $path ) {
...
if ( ! empty( $GLOBALS['WE_config']['advanced_mode'] ) && ! empty( $GLOBALS['WE_config']['cache_exception_urls'] ) ) {
	$exceptions = preg_split( '#(\n|\r)#', $GLOBALS['WE_config']['cache_exception_urls'] );

	$regex = ( ! empty( $GLOBALS['WE_config']['enable_url_exemption_regex'] ) ) ? true : false;

	foreach ( $exceptions as $exception ) {
		if ( WE_url_exception_match( $exception, $regex ) ) {
			// Exception match.
			return;
		}
	}
